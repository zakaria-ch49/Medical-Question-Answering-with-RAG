...
class TestSearchPubmed(unittest.TestCase):
    ...
    @patch("download_pubmed.requests.get")
    def test_returns_id_list_on_success(self, mock_get):
...
class TestFetchArticlesBatch(unittest.TestCase):
    ...
    @patch("download_pubmed.requests.get")
    def test_parses_xml_correctly(self, mock_get):
    ...
    @patch("download_pubmed.requests.get")
    def test_skips_articles_without_abstract(self, mock_get):
        xml_content = b"""<?xml version="1.0"?>
        <PubmedArticleSet>
          <PubmedArticle>
            <MedlineCitation>
